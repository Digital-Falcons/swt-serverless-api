...
export function buildHonoApp(
  ...
  for (const Ctor of controllers) {
    ...
    const basePath = (Reflect.getMetadata(
      META_KEYS.controller.basePath,
      Ctor
    ) || "") as string;
    // console.log('Registering controller:', Ctor.name, 'with base path:', basePath);

    const controllerMws = (Reflect.getMetadata(
      META_KEYS.controller.middlewares,
      Ctor
    ) || []) as MiddlewareHandler[];

    app.use(`${basePath}/*`, ...controllerMws); // Apply controller-level middlewares

    const routes = (Reflect.getMetadata(META_KEYS.routes, Ctor) ||
      []) as RouteDefinition[];
    ...
    for (const route of routes) {
      ...
      const statusCode =
        (Reflect.getMetadata(
          META_KEYS.method.status,
          proto,
          route.propertyKey
        ) as number | undefined) ?? route.statusCode;

      const schemas = (Reflect.getMetadata(
        META_KEYS.method.schemas,
        proto,
        route.propertyKey
      ) || route.schemas) as RouteDefinition["schemas"] | undefined;

      const paramDefs = (Reflect.getMetadata(
        META_KEYS.params,
        proto,
        route.propertyKey
      ) || []) as ParamDefinition[];
      ...
      const handler = async (c: any) => {
        try {
          ...
          if (schemas?.query) {
            ...
            queryAll = parsed.data as Record<
              string,
          ...
          const result = await instance[
            route.propertyKey as keyof typeof instance
          ](...args);
